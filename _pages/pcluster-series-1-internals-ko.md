...
트레이드오프가 있습니다. CB 인스턴스는 on-demand보다 비싸고 미리 예약해야 합니다. CB 슬롯은 인스턴스 종료 시 반환되므로, 예약 윈도우 중 롤백이나 예상치 못한 인스턴스 교체가 발생하면 회복하지 못할 수 있는 용량을 잃습니다. 활성 예약 중에는 `--rollback-on-failure false`를 사용하고 불필요한 인스턴스 사이클링을 피하세요.
