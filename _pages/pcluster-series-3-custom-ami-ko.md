...
## 반드시 깨끗한 빌드 전용 인스턴스에서 만들 것

무엇보다 먼저: AMI는 실행 중인 컴퓨트 노드가 아닌, 빌드 전용 인스턴스에서 만들어야 합니다.

이미 잘 동작하는 노드를 살짝 수정해서 스냅샷을 찍고 싶은 유혹이 생깁니다. 문제는 bootstrap이 완료된 컴퓨트 노드는 ParallelCluster가 완전히 소유한 상태라는 점입니다. slurmd가 실행 중이고, Docker가 활성화되어 있으며, post-slurmd 서비스도 떠 있습니다. cloud-init이 이미 한 번 실행되고 상태를 디스크에 남겼습니다.

이 노드를 스냅샷 찍어서 새 인스턴스를 시작하면, cloud-init이 다시 실행됩니다. 이미 부분적으로 초기화된 시스템을 다시 초기화하려고 합니다. 이 상태가 ParallelCluster의 cinc 쿡북이 기대하는 상태와 충돌합니다. 노드가 cinc finalize 단계에서 실패하고 교체 루프가 시작됩니다.

실제로 겪었습니다. 실행 중인 노드에서 빌드한 AMI(`ami-00529ac6ce247fa62`)는 매번 7분 안에 죽었습니다. 깨끗한 빌드 인스턴스로 바꾸자마자 해결됐습니다.

올바른 절차:

```bash
# 1. 공식 pcluster base AMI로 새 인스턴스 시작
#    us-east-1: ami-0f8eed74478b388d3
#    us-east-2: ami-0dc2ffd737d30ca8a

# 2. 패키지 설치 — 서비스는 시작하지 말 것
#    fabricmanager는 enable만, start 금지. Docker가 실행 중이면 안 됨.

# 3. 스냅샷 전 ami_cleanup.sh 실행
sudo /usr/local/sbin/ami_cleanup.sh

# 4. --no-reboot으로 AMI 생성
aws ec2 create-image --instance-id <ID> --name "..." --no-reboot
```

> ##### DANGER
>
> 실행 중인 컴퓨트 노드에서 절대 AMI를 빌드하지 마세요. slurmd, Docker, post-slurmd 서비스가 남긴 bootstrap 상태가 새 노드를 시작할 때마다 ParallelCluster 초기화 시퀀스와 충돌합니다.
{: .block-danger }
